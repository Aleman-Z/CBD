...
selpath = uigetdir('C:\','Select CBD Downsampled (1kHz) folder');
cd(selpath)
...
% cfold=cfold(cellfun(@(x) ~isempty(strfind(x,'.continuous')),cfold));
...
%HPC
cf1=cfold(cellfun(@(x) ~isempty(strfind(x,'HPC')),cfold));
'Loading HPC'
load(cf1{1})
LFP_HPC=HPC_downsampled;
% [LFP_HPC] = getfield(LFP_HPC,'HPC_downsampled');;

clear ans HPC_filt HPC_filt_ds HPC_downsampled
...
if downsample_data==1
    save('HPC_downsampled500.mat','HPC_downsampled')
clear HPC_downsampled

%PFC
cf1=cfold(cellfun(@(x) ~isempty(strfind(x,'PFC')),cfold));
'Loading PFC'
load(cf1{1})
LFP_PFC=PFC_downsampled;
...
save('PFC_downsampled500.mat','PFC_downsampled')
